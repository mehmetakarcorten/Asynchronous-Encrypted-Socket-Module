	...
	def getDataOrdinal(self,oldToNew=True):
		...
		for addr in list(self.keys()):
			try:
				...
				if (packet != None) and (oldToNew == True) and (packet[0] < datagram[1][0]):
...
class DataStream(list):
	...
	def getData(self):
		try: x = (self.time.pop(0),self.data.pop(0))
		except: x = None
		return x
...
class ServerNetwork(socket.socket):
	...
	async def sendAll(self, data):
		loop = asyncio.get_event_loop()
		[await loop.sock_sendall(self.connections[addr]["connection"], self.connections[addr]["encryption"].encrypt(data)) for addr in self.connections.keys()]
	...
	async def listener(self):
		while (not self.exitStatus):
			...
			self.connections[addr] = {"connection": connection, "encryption": cipher, "data": DataStream()}
